...
class NewDriver:
    ...
    def set_chrome_driver(self, version=''):
        ...
        chrome_driver = chrome.ChromeDriverManager(
            version=mobile_version).install()
    ...
    def __set_android_caps(self):
        ...
        self.__desired_capabilities[
            'chromedriverArgs'] = self.__chromedriverArgs

        self.__desired_capabilities[
            'chromeDriverPort'] = self.appium_port - 4723 + 8100
        self.__desired_capabilities[
            'systemPort'] = self.appium_port - 4723 + 8200
    ...
    def set_selenium_driver(self,
                            chrome_options=None,
                            firefox_options=None) -> TestUIDriver:
        self.__set_selenium_caps()
        self.__driver = start_selenium_driver(self.__desired_capabilities,
                                              self.__remote_url, self.__debug,
                                              self.__browser_name,
...
def __local_run(url, desired_caps, use_port, udid, log_file):
    if url is None:
        ...
        logger.log(
            f"running: appium -p {port.__str__()} -bp {bport.__str__()}")
        if udid is None:
...
def check_chrome_version(udid):
    output = subprocess.Popen([
        'adb', '-s', udid, 'shell', 'dumpsys', 'package', 'com.android.chrome',
        '|', 'grep', 'versionName'
    ],
